...
type TMinerOperation struct {
	...
	OperationType  int    `xorm:"not null comment('Operation Type: 1.proposal 3.sign up 3.vote') index index(INDEX_OPERATOR_TYPE) index(INDEX_ROUND_OPERATOR_TYPE) TINYINT(4)"`
	Round          int64  `xorm:"not null comment('Round') index(INDEX_ROUND_OPERATOR_TYPE) BIGINT(20)"`
	...
	Operator       string `xorm:"not null comment('Operator Address') index(INDEX_OPERATOR_TYPE) index(INDEX_ROUND_OPERATOR_TYPE) VARCHAR(64)"`
